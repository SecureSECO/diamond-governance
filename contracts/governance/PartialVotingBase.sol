...
/// @notice The abstract implementation of partial voting plugins.
///
/// #### Parameterization
///
    ...
    ProposalUpgradeable
{
    ...
    enum PartialVotingSettings {
        SingleVote,
        SinglePartialVote,
        MultiplePartialVote
    }

    /// @notice A container for the voting setting customizations of a proposal
    /// @param earlyExecution If the vote is sure to pass, allow it to pass before the end of the proposal.
    /// @param burnTokens If the tokens used to vote should be burned on proposal end (if the minimum participation threshold has been reached).
    /// @param partialVotingSettings If users are allowed to vote partially and if so, if they are allowed to vote multiple times.
    struct VotingMode {
        bool earlyExecution;
        bool burnTokens;
        PartialVotingSettings partialVotingSettings;
    }
    ...
    struct Proposal {
        ...
        Tally tally;
        mapping(address => IPartialVoting.PartialVote[]) voters;
        IDAO.Action[] actions;
    ...
    bytes4 internal constant PARTIAL_VOTING_BASE_INTERFACE_ID =
    ...
    function __PartialVotingBase_init(
        IDAO _dao,
    ...
    {
        return
            _interfaceId == PARTIAL_VOTING_BASE_INTERFACE_ID ||
            _interfaceId == type(IPartialVoting).interfaceId ||
            super.supportsInterface(_interfaceId);
    }

    /// @inheritdoc IPartialVoting
    function vote(
        uint256 _proposalId,
        ...
        if (!_canVote(_proposalId, account, _voteData)) {
        ...
        _vote(_proposalId, _voteData, account, _tryEarlyExecution);
    }
    ...
    function getVoteOption(
        uint256 _proposalId,
        address _voter
    ) public view virtual returns (PartialVote[] memory) {
    ...
    function canVote(
        ...
        return _canVote(_proposalId, _voter, _voteData);
    }
    ...
    function votingMode() public view virtual returns (VotingMode memory) {
    ...
    /// @param _voteData The chosen vote option and amount to be casted on proposal creation.
    ...
    function _canExecute(uint256 _proposalId) internal view virtual returns (bool) {
        ...
        if (_isProposalOpen(proposal_)) {
            // Early execution
            if (!proposal_.parameters.votingMode.earlyExecution) {
                return false;
            }
